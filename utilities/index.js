  ...
  if (data.length > 0) {
    ...
    data.forEach((vehicle) => {
      const isFavorite = favorites.some(
        (fav) => Number(fav.inv_id) === Number(vehicle.inv_id)
      );
      ...
      grid += `<div class="namePrice"><hr /><h2><a href="/inv/detail/${
        vehicle.inv_id
      }" title="View ${vehicle.inv_make} ${vehicle.inv_model} details">${
        vehicle.inv_make
      } ${vehicle.inv_model}</a></h2><span>$${new Intl.NumberFormat(
        "en-US"
      ).format(vehicle.inv_price)}</span></div>`;
      grid += `<form class="favorite-form" action="/inv/${
        isFavorite ? "remove-favorite" : "add-favorite"
      }" method="post"><input type="hidden" name="inv_id" value="${
        vehicle.inv_id
 ...
 * ************************************ */
Util.buildInventoryDetail = function (data, isFavorite = false) {
  let detail = `
  ...
  <section class="vehicle-detail">
    ...
    <div class="vehicle-info">
      ...
      <p><strong>Price:</strong> $${Number(data.inv_price).toLocaleString(
        "en-US"
      )}</p>
      <p><strong>Mileage:</strong> ${Number(data.inv_miles).toLocaleString(
        "en-US"
      )} miles</p>
      <p><strong>Color:</strong> ${data.inv_color}</p>
      <p><strong>Description:</strong> ${data.inv_description}</p>
      
      <form class="favorite-form" action="/inv/${
        isFavorite ? "remove-favorite" : "add-favorite"
      }" method="post">
...
Util.checkAccountType = (req, res, next) => {
  if (
    ...
    req.flash(
      "notice",
      "You must be an employee or admin to access this area."
    );
    return res.redirect("/account/login");
  }
};
